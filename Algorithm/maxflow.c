	...
	do {
		...
		for (n = list[u]; n != NULL; n = n->next) {
			...
			if (visited[v] == 0 && flow[u][v] < n->cap /* Node-v is unvisited && Flow is less than Cap*/) {
				queue[tail++] = v; /* Node-v is pushed */
				visited = 1; /* Node-v is visited */
			    prev[v] = u;/* prev[v] is updated */
...
int Maxflow_Update(int s, int t, int size) {
	int v = t;
	do {
		flow[prev[v]][v] += delta[t]; /* Update flow from Node-prev[v] to Node-v */
		flow[v][prev[v]] -= delta[t]; /* Update flow from Node-v to Node-prev[v] */
		v = prev[v];
	} while (v != s);
	return delta[t];
}
