...
int main()
{
	...
	for (i = 0; i < ny; i++) {
		for (j = 0; j < nx; j++) {
			if (i-1>=0 && i+1<ny && j-1>=0 && j+1<nx) {
                outimageR[i][j] = (inputimageR[i][j-1] + inputimageR[i-1][j] - 4*inputimageR[i][j] + inputimageR[i+1][j] + inputimageR[i][j+1]);
            } else {
                outimageR[i][j] = 0.0;
            }
            if (i-1>=0 && i+1<ny && j-1>=0 && j+1<nx) {
                outimageG[i][j] = (inputimageG[i][j-1] + inputimageG[i-1][j] - 4*inputimageG[i][j] + inputimageG[i+1][j] + inputimageG[i][j+1]);
            } else {
                outimageG[i][j] = 0.0;
            }
            if (i-1>=0 && i+1<ny && j-1>=0 && j+1<nx) {
                outimageB[i][j] = (inputimageB[i][j-1] + inputimageB[i-1][j] - 4*inputimageB[i][j] + inputimageB[i+1][j] + inputimageB[i][j+1]);
            } else {
                outimageB[i][j] = 0.0;
            }
		}
	...
	for (i = 0; i < ny; i++) {
		for(j = 0; j < nx; j++) {
			all_pixels++;
			if (outimageR[i][j] > (255/2) || outimageG[i][j] > (255/2) || outimageB[i][j] > (255/2)) {
				counter++;
			}
		}
	}
	...
	if (counter > (all_pixels / 100)) {
		printf("This is a picture of city.\n");
	} else {
		printf("This is a picture of sea.\n");
	}
	// printf("all_pixels:%d\n", all_pixels);
	// printf("counter:%d\n", counter);
}